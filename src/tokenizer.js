...
var tokenIDMap = JSON.stringify(tokensID).replace(/"|{|}|:[0-9]+/g, '').toLowerCase().split(',');
...
function tokenize(opt) {
  ...
  function token(data) {
    if (data.length) {
      ...
      if (!opt.stats[mode]) { opt.stats[mode] = 1; } else { opt.stats[mode]++; }
  ...
  function determine_operator(buf) {
    ...
    do {
      var lenOp = l + j;
      var op = lenOp === 1 ? buf[0] : buf.slice(0, lenOp);
      ...
      content = l === 0 ? '' : content.slice(lenOp);
      return l;
    } while (1);
  }
  ...
  function tokenStringMode(contentstr) {
    ...
    } else if (allBuiltinsDict[contentstr]) {
  ...
  return function (data) {
    ...
    if (data.replace) {
      ...
      while (i < len) {
        ...
        c = data[i];
        switch (mode) {
          case tokenID.BLOCK_COMMENT: i = block_comment(); break;
          case tokenID.LINE_COMMENT: i = line_comment(); break;
          case tokenID.PREPROCESSOR: i = preprocessor(); break;
          case tokenID.OPERATOR: i = operator(); break;
          case tokenID.INTEGER: i = integer(); break;
          case tokenID.HEX: i = hex(); break;
          case tokenID.FLOAT: i = decimal(); break;
          case tokenID.TOKEN: i = readtoken(); break;
          case tokenID.WHITESPACE: i = whitespace(); break;
          case tokenID.NORMAL: i = normal(); break;
          default:
            continue;
        }
